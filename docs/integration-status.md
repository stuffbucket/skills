...
Current state of each integration surface as of May 2026.
...
The MCP server (`stuffbucket`) exposes a skill-router instead of registering each skill as a separate tool. This keeps context window cost constant regardless of how many skills exist.
...
### How an AI assistant uses a skill
...
4. The server resolves `{{SKILL_DIR}}` to the absolute skill path before returning content
5. The agent follows the instructions and uses any `allowed-tools`
6. If the skill references bundled scripts/references/assets, those paths are absolute and work from any CWD

### How MCP connects tools to editors

1. Editor reads `.mcp.json` (or `.vscode/mcp.json`)
2. Editor launches the `stuffbucket` MCP server via `npx -y -p @stuffbucket/skills stuffbucket-skills`
